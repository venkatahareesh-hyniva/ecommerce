...
export const addToCart = async (req: any, res: any) => {
  try {
    ...
    if (result.type === "productNotFound") {
      return sendNotFound(res, "Product not found");
    }

    if (result.type === "insufficientStock") {
      return sendBadRequest(res, "Insufficient stock");
    }
...
export const updateCartItem = async (req: any, res: any) => {
  try {
    ...
    const result = await cartService.updateCartItem(userId, items);

    if (result.type === "notFound") {
      return sendNotFound(res, "Cart not found");
    }

    if (result.type === "itemNotFound") {
      return sendNotFound(res, `Product ${result.productId} not found in cart`);
    }
    ...
    return sendSuccessResponse(res, "Cart updated successfully", result.data);
  } catch (error) {
